...
\wrpdef{Euler's Phi Function}{The function \(\phi(m)\) defined by the rule \[\phi(m) = |\{0 \leq a < m \mid \gcd(a,m) = 1\}| \]}
...
\lesol{We can look to the \hyperlink{thm:Primitive Root Theorem}{Primitive Root Theorem} to see how many primitive roots are \(7\) has. Because 7 is prime, we know that \(\phi = 6\). Thus, we know that 7 will have 6 primitive roots. Because there are 6 primitive roots in total, and \(\F_7^*\) has 7 elements (including 0), we know that 1-6 will be the primitive roots.}
