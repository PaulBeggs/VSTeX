...
\sol{
    \begin{enumerate}
        ...
        \item \textit{Proof} Following a similar process to \((b)\), we start with \(g^{n\log_g(h)}\). Then, by using the properties of exponents, we can rewrite this as \(g^{\log_g(h)^n}\). Since \(g^{\log_g(h)^n} = h^n\). Putting everything together, we have \begin{align*}
