...
\begin{theorem}
    {Primitive Root Theorem}Let \(p\) be a prime number. Then there exists an element \(g \in \F_p^*\) whose powers give every element of \(\F_p^*\), i.e., \[\F_p^* = \{1,g,g^2,g^3.\dots,g^{p - 2}\}.\] Elements with this property are called the \textit{primitive roots of} \(\F_p\) or \textit{generators of} \(\F_p^*\). They are elements of order \(p - 1\).
\end{theorem}
...
\lesol{We can look to the \hyperref[thm:Primitive Root Theorem]{Primitive Root Theorem} to see how many primitive roots \(7\) has. Because 7 is prime, we know that \(\varphi = 6\). Thus, we know that 7 will have 6 primitive roots. Because there are 6 primitive roots in total, and \(\F_7^*\) has 7 elements (including 0), we know that 1-6 will be the primitive roots.}
