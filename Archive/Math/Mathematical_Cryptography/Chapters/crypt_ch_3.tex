...
\begin{corollary}
    {1: Euler's Totient Theorem for phi}If \(N\) is prime, \(a^{\varphi(N)} \equiv 1 \ (\text{mod } N)\). For \(\varphi(N)\), we can express it as \(N-1\) from \hyperref[thm:Fermat's Little Theorem]{FLT}.
\end{corollary}
...
\begin{corollary}
    {2: Euler's Totient Theorem for phi}If \(p,q\) are distinct primes, then \[
...
\begin{example}
    {ETT for phi}Find \(a^x \ (\text{mod } 15)\).
\end{example}
...
\noindent \textit{Proof.} We claim that \(m' = m\).
\begin{align*}
...
\lesol{ \vspace{0.5cm} \newline
   ...
   \noindent Thus, we found that 421 is a factor of 540143.
}
...
\begin{exercise}
    {3.24} {For each of the following numbers \(N\), compute the values of \(N + 1^2, N + 2^2, N + 3^2, N + 4^2\),\dots as we did in Example 3.34 (\hyperref[ex:3.11]{Example 3.11 in notes}), until you find a value \(N + b^2\) that is a perfect square \(a^2\). Then use the values of \(a\) and \(b\) to factor \(N\).}\vspace*{-0.1cm}
    \begin{center}
        (a) \(N = 53357\) \qquad (b) \(N = 34571\)
    \end{center}
\end{exercise}
...
\lesol{
    ...
    \begin{align*}
        ...
        25217 + 8^2 & = 25281 = 159^2 & \text{Eureka! \textbf{a square!}}
    \end{align*}
