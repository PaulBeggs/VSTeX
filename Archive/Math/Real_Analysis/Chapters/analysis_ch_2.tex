...
\begin{tcolorbox}%
    [enhanced, sharp corners=downhill, colframe=draculared, colback=draculared!15!draculabg, coltitle=white, coltext=draculafg, label=def:2.2.3B, title={\large \textbf{Definition 2.2.3B}},attach boxed title to top right={xshift=-0.5cm}, boxed title style={colback=draculared}]
...
\begin{example}
    {Limit Proof 2}Claim: \(\displaystyle \lim_{n \rightarrow \infty} \frac{2n-3}{2n} = 1\)
\end{example}
...
\begin{example}
    {Limit Proof 3}Claim: \(\displaystyle \limn \frac{2n^2 + 1}{n^2} = 2\)
\end{example}
...
\begin{align*}
    \disabs{\frac{2n^2 + 1}{n^2} - 2} & = \frac{1}{n^2}                                \\
                                      & \leq \frac{1}{N^2}                             \\
                                      & < \frac{1}{\left(1/(\sqrt{\epsilon})\right)^2} \\
                                      & = \frac{1}{1/\epsilon}                         \\
                                      & = \epsilon
\end{align*}
...
\begin{example}
    {Limit Proof 4}Claim: \(\displaystyle \limn \frac{7n + 8}{3n + 6} = \frac{7}{3} \)
\end{example}
...
\epf{
    ...
    \end{align*}
    \(** \frac{18}{9n + 8} < \frac{18}{9n} < \epsilon \Rightarrow \frac{2}{\epsilon} < N\). \(\exists N \in \N\) such that \(N > \frac{2}{\epsilon}\). Assume \(n \geq N\),
    \begin{align*}
...
\begin{example}
    {Limit Proof 5}Let \((x_n) = \displaystyle \frac{\cos(n)}{3n}\). Claim: \(\displaystyle \limn (x_n) = 0\)
\end{example}
...
\begin{example}
    {Limit Proof 6}Let \(\displaystyle (y_n) = \frac{4n-1}{n^2}\). Claim: \(\displaystyle \limn y_n = 0\).
\end{example}
...
%               \innerpf{
%               For \(n \geq 6\), we have \(\displaystyle \frac{5}{n} \leq \frac{5}{6} < 1\), so \(a_{n} = [[5/n]] = 0\).
...
\begin{itemize}
    ...
    \item If \(c > 0\): \\
          \(\disabs{ca_n - ca} = \disabs{c}\disabs{a_n - a} < \disabs{c}\frac{\epsilon}{\disabs{c}} = \epsilon\).
\end{itemize}
...
\tpf{
Let \((a_n)\) be a contracting sequence; that is, there exists a constant \(0 < C < 1\) such that for all \(n \in \mathbb{N}\),
\[
    |a_{n+1} - a_n| \leq C |a_n - a_{n-1}|.
\]
We will show that \((a_n)\) is a Cauchy sequence.

First, we observe by induction that for all \(k \geq 1\),
\[
    |a_{n+k} - a_{n+k-1}| \leq C^{k} |a_n - a_{n-1}|.
\]
\textit{Proof by induction}:

\textbf{Base case \((k=1)\):}
\[
    |a_{n+1} - a_{n}| \leq C |a_n - a_{n-1}|.
\]

\textbf{Inductive step:} Assume that for some \(k \geq 1\),
\[
    |a_{n+k} - a_{n+k-1}| \leq C^{k} |a_n - a_{n-1}|.
\]
Then,
\begin{align*}
    |a_{n+k+1} - a_{n+k}| & \leq C |a_{n+k} - a_{n+k-1}|                \\
                          & \leq C \left( C^{k} |a_n - a_{n-1}| \right) \\
                          & = C^{k+1} |a_n - a_{n-1}|.
\end{align*}
Thus, the inequality holds for \(k+1\), completing the induction.

Next, for any integers \(m > n\), we have:
\[
    |a_m - a_n| = \left| \sum_{j=n}^{m-1} \left( a_{j+1} - a_j \right) \right| \leq \sum_{j=n}^{m-1} |a_{j+1} - a_j|.
\]
Applying the inequality obtained from the induction,
\[
    |a_{j+1} - a_j| \leq C^{j - n +1} |a_n - a_{n-1}|.
\]
Therefore,
% \begin{align*}
%     |a_m - a_n| & \leq |a_n - a_{n-1}| \sum_{j=n}^{m-1} C^{j - n +1}                                 \\
%                 & = |a_n - a_{n-1}| \sum_{k=1}^{m - n} C^{k} \quad (\text{Let } k = j - n +1 \text)} \\
%                 & = |a_n - a_{n-1}| \left( \frac{C (1 - C^{m - n})}{1 - C} \right)
% \end{align*}
Since \(C^{m - n} \geq 0\), we have:
\[
    |a_m - a_n| \leq |a_n - a_{n-1}| \left( \frac{C}{1 - C} \right).
\]
As \(n \to \infty\), the term \(|a_n - a_{n-1}|\) tends to zero because:
\[
    |a_n - a_{n-1}| \leq C^{n-1} |a_1 - a_0| \to 0 \quad \text{as } n \to \infty.
\]
Therefore, for any \(\epsilon > 0\), there exists \(N \in \mathbb{N}\) such that for all \(n \geq N\),
\[
    |a_n - a_{n-1}| < \epsilon \left( \frac{1 - C}{C} \right).
\]
Then, for all \(m, n \geq N\) (with \(m > n\)),
\[
    |a_m - a_n| \leq |a_n - a_{n-1}| \left( \frac{C}{1 - C} \right) < \epsilon.
\]
This shows that \((a_n)\) is a Cauchy sequence. Since every Cauchy sequence in \(\mathbb{R}\) converges, the sequence \((a_n)\) converges.
}
