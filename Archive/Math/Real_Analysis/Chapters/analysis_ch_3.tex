...
\begin{definition}
    For a point \(x \in \R\), and \(\epsilon > 0\), we define the \textit{epsilon-neighborhood} of \(x\) to be \(V_\epsilon(x) = \{y \in \R \mid |x - y| < \epsilon\}\). In other words, \(V_{\epsilon}(x)\) is the open interval \((x - \epsilon, x + \epsilon)\), centered at \(x\) with radius \(\epsilon\).
\end{definition}
...
\tpf{ The first set of ``if and only if proofs'' will be to prove (i) and (ii) are equivalent. Then, we will prove (ii) and (iii) are equivalent. 
    \begin{customframedproof}[linecolor=draculablue]
        \begin{proofpart}[draculablue]{(\(\Rightarrow\))}
        ...
        \end{proofpart}
    \vspace{2mm}
            \begin{proofpart}[draculablue]{(\(\Leftarrow\))}
