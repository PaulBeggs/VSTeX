...
\begin{enumerate}
    ...
    \sol{
        \[
            E(X) = \int_0^\infty \max(t,2) \cdot \frac{1}{3} e^{-\frac{t}{3}} dt = \int_0^2 2 \cdot \frac{1}{3} e^{-\frac{t}{3}} \, dt + \int_2^\infty t \cdot \frac{1}{3} e^{-\frac{t}{3}} \, dt \approx 3.540.
        \]
    }
    \item (1 point each) An actuary determines that the annual number of tornadoes in counties \(P\) and \(Q\) are jointly distributed as follows: \\[0.5em]
    ...
    \begin{enumerate}
        ...
        \sol{
            \[
                E(Q|P=0) = 0 \cdot \frac{0.12}{0.25} + 1 \cdot \frac{0.06}{0.25} + 2 \cdot \frac{0.05}{0.25} + 3 \cdot \frac{0.02}{0.25} = 0.88.
            \]
        }
    ...
    \begin{enumerate}
        \item (2 points) Explicitly list the 9 points which are in the support (i.e. the sample space):
        \sol{
            \(S = S_{X} \cdot S_{Y}\). \(S_{Y} = \{1, 2, 3\}\) for \(x = 1\), \(S_{Y} = \{0, 1, 2, 3\}\) for \(x = 2\), and \(S_{Y} = \{0, 1, 2\}\) for \(x = 3\). Thus, \(S = \{(1,1), (1,2), (1,3), (2,0), (2,1), (2,2), (2,3), (3,0), (3,1), (3,2)\}\)
        }
    ...
    \end{enumerate}
    \item (2 points each) Let \(X\) be the weight of robin eggs, in grams, and \(Y\) be the daily high temperature, in degrees Celsius. Assume that \(X\) and \(Y\) have a bivariate normal distribution with \(\mu_{X} = 145.2\), \(\sigma_{X}^{2} = 109.2\), \(\mu_{Y} = 23.5\), \(\sigma_{Y}^{2} = 21.8\), and \(\rho = -0.34\).
    \begin{enumerate}
