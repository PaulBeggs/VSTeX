...
\wrpprop{3.17}{Let \(p\) be an odd prime, and write \(p - 1 = 2^kq\) with \(q\) being odd. Then, let \(a\) be any number not divisible by \(p\). Then, one of the following must hold: \begin{enumerate}[label=(\roman*)]
    ...
    \end{enumerate}} Further, if one of these 3 are congruent, then the previous would be \(\pm 1\). So either,
...
\lesol{
    \begin{enumerate}
        ...
        \item Pick \(a = 7\). Start by solving \(7^{35} \pmod{561} \equiv 241\). If it was congruent to 1, then \(N\) is a possible prime. Then, if \(7^{35} \equiv -1\), then N is a possible prime.
        \item If the number is neither congruent to 1 or \(-1\), keep squaring and checking.
              \begin{itemize}
                  \item Like this: \((7^{36})^2 \pmod{561} \equiv 298\). If this number is congruent to 1, then we are done.
              \end{itemize}
        \item If it is congruent to \(-1\), then we choose another \(N\), and note this is a possible prime. If congruent to \(k\), keep going.
        \item Thus, \((7^{36})^4 \pmod{561} \equiv 166\), \((7^{36})^4 \pmod{561} \equiv 67\), and \((7^{36})^{16} \pmod{561} \equiv 1\), so 561 is composite and 7 is a \hyperlink{Miller-Rabin Witness}{M-R witness}.
    \end{enumerate}
}
