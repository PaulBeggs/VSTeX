...
\begin{enumerate}
    \item Pirates hid a treasure somewhere on campus. They broke the location up into two pieces\points{12} and encrypted them. An ElGamal Encryption scheme with the public key \(p = 24691\), \(g = 106\), and \(A = 12375\) was used to encrypt the first part of a hidden message. Here is the resulting ciphertext: \(c_1 = 3799\), \(c_2 = 5973\).
          \begin{enumerate}
              ...
              \item Use the private key found in part (a) to decrypt the ciphertext (be sure to convert from ascii back to text). Note: This will be the first part of a message that will continue in the next questions so the last character will be a hyphen.

                    %% SOLUTION HERE! %%
                    \sol{
                        to decrypt the message, we will compute \((c_1^a)^{-1} \cdot c_2 \pmod{p}\). Substituting values, we get \((3799^{22392})^{-1} \cdot 5973 \pmod{24691} \equiv 7378\). From ascii translation, we get \texttt{IN}.
                    }
    ...
    \item The pirates used an RSA Encryption scheme with public key \(N = 57247159\) and \(e = 11\) \points{12}to encode the second part of the message. Here is the resulting cipher text: \(c = 2772237\).
          \begin{enumerate}
              \item Use your knowledge of algorithms to factor \(N\). Show all work.
                    ...
                    %% SOLUTION HERE! %%
            \sol{
                If we use an method like factoring by difference of squares, we can set up a brute force algorithm to decrypt the factors of \(N\). Thus, we find a value \(k\) such that \(N + k^2 = m^2\) for some \(m \in \Z\). This is equal to \(N = m^2 - k^2 = (m+k)(m-k)\). The brute force algorithm will iterate through values for k until it finds a square \(m^2\). To do this in Python, we can look to the code below. From the algorithm we get \((p,q) = (421, 135979)\)
            }
...
\end{lstlisting}
              \item Use the factors of \(N\) from part (b) to find the private decryption exponent \(d\).
                    ...
                    %% SOLUTION HERE! %%
                \sol{
                We can compute \(d\) to be \(d = e^{-1} \pmod{420 \cdot 135978} = 11^{-1} \pmod{57110760} \equiv 36343211\)
                }
