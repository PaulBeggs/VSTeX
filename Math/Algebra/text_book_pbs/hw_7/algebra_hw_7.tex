...
In Exercises 1, 3, 4, and 7, classify the given group according to the fundamental theorem of finitely generated abelian groups.
...
\begin{enumerate}
    ...
    \sol{
        \(|\gen{(1,2)}| = 8 \implies |(\Z_{4} \times \Z_{8}) / \gen{(1,2)}| = 8\). Therefore, either \(\Z_{8}\) or \(\Z_{4} \times \Z_{2}\). Since \(|(0,1) + \gen{(1,2)}| = 8\), \((\Z_{4} \times \Z_{8}) / \gen{(1,2)} \simeq \Z_{8}\).
    }
    \setcounter{enumi}{6}
    \item \((\Z \times \Z) / \gen{(1,2)}\)
    \sol{
        In the factor group, everything in the subgroup becomes the identity, so \((1,2) = (0,0)\). This implies \(x + 2y = 0\), or \(x = -2y\). Therefore, every element in the factor group can be written as \((-2y,y) = y(-2,1)\). Since \(x\) depends on \(y\), the factor group is isomorphic to \(\Z\). 
    }
...
\begin{enumerate}
    \setcounter{enumi}{19}
    \item Let \(K\) be the subgroup of \(F\) consisting of the constant functions. Find a subgroup of \(F\) to which \(F / K\) is isomorphic.
    \sol{
        Define a homomorphism \(\varphi \funccolon F \to F\) by \(\varphi(f) = f(x) - f(0)\). The kernel of this map consists of all functions where \(f(x) - f(0) = 0\), which implies \(f(x)\) is constant, so \(\ker(\varphi) = K\). The image of the map, \(\varphi[F]\), is the set of all functions that evaluate to 0 at \(x = 0\). By the fundamental theorem of homomorphisms, \(F/K\) is isomorphic to the subgroup \(\{f \in F \mid f(0) = 0\}\).
    }
\newpage
    ...
    \sol{
        This problem is almost exactly like the last one: Define a homomorphism \(\varphi \funccolon F^{*} \to F^{*}\) by \(\varphi(f) = f(x) / f(0)\). The kernel of this map consists of all functions where \(f(x) / f(0) = 1\), which implies \(f(x)\) is constant, so \(\ker(\varphi) = K^{*}\). The image of the map, \(\varphi[F^{*}]\), is the set of all functions that evaluate to 1 at \(x = 0\). By the fundamental theorem of homomorphisms, \(F^{*}/K^{*}\) is isomorphic to the subgroup \(\{f \in F^{*} \mid f(0) = 1\}\).
    }
    \setcounter{enumi}{27}
    ...
    \pf{
        The group \(G = \Z\) contains only the identity element, 0, with finite order. Every other integer has infinite order. Thus, \(G\) has no elements of finite order \(> 1\). Now, consider \(G / H\) where \(H = 2\Z\). Since this factor group has order 2, it is isomorphic to \(\Z_{2}\). Then, since \(\Z_{2}\) is itself finite, all of its elements must also have finite order.
    }
    ...
    \pf{
        Let \(G = \Z_{4} \times \Z_{2}\). Consider the subgroups \(H = \gen{(2,0)}\) and \(K = \gen{(0,1)}\). Both \(H\) and \(K\) are subgroups of order 2. Since there is only one group of order 2 up to isomorphism, \(H \simeq K\). Notice, however, that \(G/H \simeq \Z_{2} \times \Z_{2}\), while \(G/K \simeq \Z_{4}\). Therefore, \(G/H \not\simeq G/K\). 
    }
\end{enumerate}
