...
\newcommand{\assignmentname}{Homework 5: Sections 13 \& 14}
...
% Let¢ : Z::9 ----;. Z::2 be given by q:,(x) = th e remainder of x when divided by 2, as in the division algorithm .
...
\begin{enumerate}
    \setcounter{enumi}{18}
    \item \(\ker(\varphi)\) and \(\varphi(20)\) for \(\varphi \funccolon \Z \to S_{8}\) such that \(\varphi(1) = (1,4,2,6)(2,5,7)\).\footnote{I decided not to capitalize the `k' in ``ker'' to keep with \texttt{amsmath}'s \textbackslash\texttt{ker} command.}
    \sol{
...
\begin{enumerate}
    ...
    \setcounter{enumi}{48}
% Show that if G, G', and G11 are groups and if ¢ : G ----;. G' and y : G' ----;. G11 are homomorphisms, then the
% composite map yep : G ----;. G11 is a homomorphism .
...
In Exercises 3 and 6, find the order of the given factor group.
...
In Exercises 10 and 15, give the order of the element in the factor group.
...
\begin{enumerate}
    ...
    \setcounter{enumi}{14}
    \item \((2, 0) + \gen{(4, 4)}\) in \((\Z_6 \times \Z_5) / \gen{(4, 4)}\)
    \sol{
