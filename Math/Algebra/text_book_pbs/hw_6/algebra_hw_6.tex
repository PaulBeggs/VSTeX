...
\begin{enumerate}
    ...
    \sol{
        ...
        \begin{itemize}
            ...
            \[
                L = pKp^{-1} = pgHg^{-1}p^{-1}.
            \]
            Notice \(pg = g''\) and \(g^{-1}p^{-1} = (pg)^{-1} = g''\). The product \(pg\) and \((pg)^{-1}\) are both in \(G\) because \(G\) is a group. Thus, we have shown that \(H\) is conjugate to \(L\). Therefore, the relation is transitive.
        \end{itemize}
    }
    ...
    \sol{
        ...
        \[
            ((aH)^{k})^{q} = H^{q} = H.
        \]
        Using the definition of coset multiplication, we know that \((aH)^{m} = a^{m}H\). Therefore, we have shown that \(a^{m}H = H\). By the properties of cosets, \(gH = H\) implies \(g \in H\). Consequently, \(a^{m} \in H\).
    }
