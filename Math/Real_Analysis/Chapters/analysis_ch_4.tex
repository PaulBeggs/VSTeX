...
\begin{definition}
    A function \(f \colon A \to \R\) is \textit{\underline{not} uniformly continuous} on \(A\) if there exists \(\epsilon_{0} > 0\) such that for all \(\delta > 0\), there exists \(x, c \in A\) such that \(\disabs{x - c} < \delta\) and \(\disabs{f(x) - f(c)} \geq \epsilon_{0}\).
\end{definition}
...
\tpf{
    Without the loss of generality, assume \(f(a) < f(b)\) and let \(y \in (f(a),f(b))\). Let \(I_{1} = [a_{1},b_{1}]\). Bisect \(I_{1}\) into two intervals \([a_{1},d]\) and \(d,b_{1}\) where \(d = \frac{a_{1} + b_{1}}{2}\). 
\begin{itemize}
...
Since \(f\) is continuous, we have
\[
    f(c) = \lim_{n \to \infty} f(a_{n}) \leq y \qquad \text{and} \qquad f(c) = \lim_{n \to \infty} f(b_{n}) \geq y.
\]
Therefore, \(f(c) = y\).
}
