...
\expf{
    Look at the differences of consequtive terms, \(\disabs{a_1 - a_2} = 1\), \(\disabs{a_2 - a_3} = 1/2\), we can see a formula \(a_n - a_{n+1} = {1/2}^{n-1}\). Assume \(\disabs{a_n - a_m}= \disabs{a_n - a_{n+1} - a_{n+2}} - \cdots - a_{m - 1} - a_m\) with \(n <m\). From the \hyperlink{Triangle Inequality}{Triangle Inequality},
    \begin{align}
    ...
    Now, conclude the proof by letting \(\epsilon > 0\). We know \((1/2^n) \rightarrow 0\). Thus, there exists \(N \in \N\) such that \(\frac{1}{2^N} < \epsilon\). For all \(n,m \geq N\), (without loss of generality \(n < m\)) \(\disabs{a_n - a_m} < \frac{1}{2}n \leq \frac{1}{2}N < \epsilon\). Therefore, \(a_n\) is \hyperlink{def:2.6.1}{Cauchy} and it converges.
}
