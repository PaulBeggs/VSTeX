...
We select our orientation as follows:
\begin{itemize}
...
This is (somewhat) easier to calculate than its scalar equivalent, typically. For all surface integrals, you are strongly advised to draw a picture and think a bit before just blindly trying to guess about your situation.
...
Suppose that \(S\) is a closed surface – that it, it contains some volume. Then, since \(S\) has no boundary curve,
\[
...
\begin{itemize}
    ...
    \item A vector field which is the curl of some other vector field is sometimes called \cyanit{solenoidal} – the flux surface integral of a solenoidal field over a closed surface is always 0.
...
Let \(S_{a}\) be the boundary of \(B_a\), and then the flux across \(S\) is
\begin{align*}
...
Each of the following can be interpreted as a version of the Fundamental Theorem of Calculus:
\begin{itemize}
...
% The divergence of the magnetic field is zero, which says that magnetic monopoles do not exist! The net flux
% of the magnetic field through any closed surface is 0, and so there is no free magnetic “charge.”
% Faraday’s Law of Induction
% The circulation (the “electromotive force”) around a closed loop matches the time-changing flux of the
% magnetic field through any surface bounded by that loop. Or, a changing magnetic field will induce a
% current along a closed loop. A time-varying magnetic field will always create a non-conservative electric
% field, and so work can be performed in on charges in a closed loop.
% Ampere’s Law
% The flux of the current density and the time-varying change of the flux in the electric field each produce
% magnetic circulation in a closed loop. This explains why the total current in a surface enclosed by the loop
% is proportional to the magnetic circulation.