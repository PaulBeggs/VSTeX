...
\tpf{
    ...
    The \namrefthm{Nested Interval Property} states that \(\displaystyle \bigcup_{n = 1}^\infty I_n\) is non empty. Let \(L\) be a point in \(\displaystyle \bigcup_{n = 1}^\infty I_n\). We claim \(\limn{x_{n_k}} = L\). Let \(\epsilon > 0\). There exists an \(N \in \N\) such that \(\frac{2M}{2^n} < \epsilon\). (Since \(\limn{\frac{2M}{2^n}} = 0\). See ) For any \(k \geq N\), recall that \(x_{n_k}\), \(L \in I_k\). Since \(I_k\) has length \(\frac{2M}{2^n}\). Thus, \(\disabs{x_{n_k} - L} < \epsilon\). Therefore, \(\limn{x_{n_k}} = L\) and \((x_n)\) has a convergence sequence. 
}
...
\begin{tikzpicture}
    ...
    \def\halfM{\M / 2}
    \draw[thick] (-\halfM, -1) -- (\halfM, -1) node[anchor=west] {$I_1$};
    \draw[thick] (-\halfM, -0.9) -- (-\halfM, -1.1) node[below] {};
    \draw[thick] (\halfM, -0.9) -- (\halfM, -1.1) node[below] {};
    
    % Left and Right halves
    \draw[dashed] (-\M, -1.5) -- (0, -1.5) node[anchor=west] {$I_L$};
    \draw[dashed] (0, -1.5) -- (\M, -1.5) node[anchor=west] {$I_R$};
    
    % Draw I_2 within I_1
    \def\quarterM{\M / 4}
    \draw[thick] (-\quarterM, -2) -- (\quarterM, -2) node[anchor=west] {$I_2$};
    \draw[thick] (-\quarterM, -1.9) -- (-\quarterM, -2.1) node[below] {};
    \draw[thick] (\quarterM, -1.9) -- (\quarterM, -2.1) node[below] {};
    
    % Further subintervals
    \def\eighthM{\M / 8}
    \draw[thick] (-\eighthM, -3) -- (\eighthM, -3) node[anchor=west] {$I_3$};
    \draw[thick] (-\eighthM, -2.9) -- (-\eighthM, -3.1) node[below] {};
    \draw[thick] (\eighthM, -2.9) -- (\eighthM, -3.1) node[below] {};
    
    % Points representing x_n's
    \filldraw[red] (1.5, 0) circle (2pt) node[anchor=north] {$x_{n_1}$};
    \filldraw[red] (0.7, -1) circle (2pt) node[anchor=north] {$x_{n_2}$};
    \filldraw[red] (0.35, -2) circle (2pt) node[anchor=north] {$x_{n_3}$};
    \filldraw[red] (0.18, -3) circle (2pt) node[anchor=north] {$x_{n_4}$};
