...
\sol{
    \begin{enumerate}
        \setcounter{enumi}{1}
        \item \(\limn{\frac{2n^2}{n^3+3}} = \text{ the answer }\). Question: Should we use \defref{2.2.3}, or the \namrefthm{Archimedean Principle}?
    \end{enumerate}
}
...
\tpf{(i) \\
    ...
    Let \(\epsilon > 0\). Since convergent sequences are bounded, then there exists $M > 0$ such that for all $n \in \N$, $|a_n| \leq M$. We can choose \(M\) so that \(|b_n| \leq M\) as well. Since $(a_n)$ converges to $a$, there exists $N_1 \in \N$ such that for all $n \geq N_1$, $\disabs{a_n - a} < \frac{\epsilon}{2M}$. Since $(b_n)$ converges to $b$, there exists $N_2 \in \N$ such that for all $n \geq N_2$, $\disabs{b_n - b} < \frac{\epsilon}{2M}$. Now, let $N = \max\{N_1, N_2\}$. Thus, for any $n \geq N$, (refer back to scratch paper, and change (2.1)'s sign from an `=' to `$\leq$'). \\
