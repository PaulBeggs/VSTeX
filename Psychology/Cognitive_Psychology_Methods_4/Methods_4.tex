...
\begin{enumerate}
	...
	\sol{
		...
		Thus, I redid the problem, and this time when the rule said ``to the left (or right),'' I made sure to add the requisite content to both columns (if applicable) instead of one. After changing up my strategy and by interpreting the rules in a different way, I was able to solve the problem. 

		For the second task I really struggled. I read the directions in every way except the correct way. It wasn't until I showed the problem to my friend, who had an \textit{insight}---a sudden realization of an interpretation that was not initially obvious---that I was able to solve it. After that, I went through each interval that made up \(1 \leq 100\), and I was able to visualize it in a different way, like in \hyperref[tab:table4]{Table 4}.
	}
...
\begin{enumerate}
	...
	\sol{
		First, this task engages \textit{controlled processes}---associated with how you manage your attention, your ability to \textit{task-switch} (consciously move from one task to another), and \textit{working memory}---by requiring me to give my attention to the task at hand. Additionally, when the task may be too difficult, it allows me to move from one list to another. When I bounce from list to list, I am using my \textit{associative processes}---ability to retrieve associations from memory---to try and find a connection between the words. Thus, this task required me to use a combination of both processes to solve it. 
	}
	\item Evaluate this task. Is it a good test of creativity? Why or why not? \textbf{(3 pts)}\\[0.25cm]
	\sol{
		Yes, it was a good test of creativity. It required a lot of deep thinking, and took time to solve. It was not extremely easy, or extremely hard. It was a good balance of difficulty, and required me to think in a different way than I was used to.
	}
\end{enumerate}
