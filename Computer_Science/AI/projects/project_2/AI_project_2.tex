...
public class NegaMax extends CheckersSearcher {
    ...
    public Optional<Duple<Integer, Move>> selectMove(Checkerboard board) {
        ...
        for (Move move : board.getCurrentPlayerMoves()) {
            ...
            int scoreForCurrentPlayer = getScoreForCurrentPlayer(depth, alternative, currentPlayer);

            if (scoreForCurrentPlayer > bestScore) {
                bestScore = scoreForCurrentPlayer;
                bestMove = move;
            }
        }
    ...
    private int maxScore(Checkerboard board, int depth) {
        ...
        for (Checkerboard alternative : board.getNextBoards()) {
            ...
            maxScore = Math.max(maxScore, scoreForCurrentPlayer);
        }
        return maxScore;
    }

    private int getScoreForCurrentPlayer(int depth, Checkerboard alternative, PlayerColor currentPlayer) {
        PlayerColor nextPlayer = alternative.getCurrentPlayer();
        int score = maxScore(alternative, depth - 1);

        int scoreForCurrentPlayer;
        if (currentPlayer == nextPlayer) {
            scoreForCurrentPlayer = score;
        } else {
            scoreForCurrentPlayer = -score;
        }
        return scoreForCurrentPlayer;
    }
}
\end{minted}
